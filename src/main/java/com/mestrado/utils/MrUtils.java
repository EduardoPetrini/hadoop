...
public class MrUtils {
		...
		try {
			...
			if (fs.isDirectory(p)) {
				...
				for (FileStatus f : ff) {
					...
					if (aux.getName().contains("output") || aux.getName().contains("inputCached") || aux.getName().contains("candidatosTxt")
							|| aux.getName().contains("outputCandidates") || aux.getName().contains("inputCandidates")) {
	...
	public static void printConfigs(Main m) {
		...
		System.out.println("Entry dir: " + Main.inputEntry);
	...
	public static void saveSequenceInHDFS(ArrayList<String> data, String fileOut) {
		...
		try {
			SequenceFile.Writer writer = SequenceFile.createWriter(c, SequenceFile.Writer.file(p), SequenceFile.Writer.keyClass(Text.class),
	...
	public static void copyToInputGen(String outputCount) {
		...
		try {
			...
			// for(FileStatus fst: files){
			// if(fst.getPath().getName().startsWith("part")){
			// // copy(FileSystem srcFS, FileStatus srcStatus, FileSystem dstFS,
			// Path dst, boolean deleteSource, boolean overwrite, Configuration
			// conf)
			// if(FileUtil.copy(fs, fs.getFileStatus(fst.getPath()), fs, input,
			// false, true, c)){
			// System.out.println("Copiado "+fst.getPath().getName()+" para
			// "+input.getName());
			// }else{
			...
			if (fs.isDirectory(p)) {
				...
				for (FileStatus status : fss) {
					if (status.getPath().getName().contains(cCurrent)) {
						System.out.println("Lendo itemsets from sequence file \""+Main.inputCandidatesDir+"/"+status.getPath().getName()+"\"");
						itemsets.addAll(readSequenfileInHDFS(Main.inputCandidatesDir+"/"+status.getPath().getName()));
					}
